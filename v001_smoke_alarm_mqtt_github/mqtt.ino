void MQTT_Connect()
{
  ...
  */
  if (MQTTclient.connect(mqtt_client_id.c_str(), mqtt_username, mqtt_password, (base_mqtt_topic + "/Status").c_str(), 0, 1, "Dead"))
  {
    ...
    MQTTclient.publish((base_mqtt_topic + "/alarm").c_str(), String("false").c_str(), true);   //set alarm to false initially
    MQTTclient.loop();


    // ... and then re/subscribe to the watched topics
    //MQTTclient.subscribe((base_mqtt_topic + "/TOPIC").c_str());   // Watch the .../TOPIC topic for incoming MQTT messages
    //MQTTclient.loop();
